...
Double_t roostats_clmFast(Double_t ilum, Double_t slum,
                       Double_t eff, Double_t seff,
                       Double_t bck, Double_t sbck,
                       Int_t n,
                       Bool_t gauss = kFALSE,
...
//  ws->SaveAs("ws.root");
 ...
 ModelConfig* mc = (ModelConfig*) ws->obj("BModel");

  ProfileLikelihoodCalculator pl(*data,*mc);
  pl.SetConfidenceLevel(0.50); // 95% interval
...
//  }
  ...
  return (interval->UpperLimit(*firstPOI) - interval->LowerLimit(*firstPOI)) / 2.0;
}
