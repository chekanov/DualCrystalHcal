...
namespace CalVision   {
  ...
  void DualCrystalHcaldump_branch(int num_evts, TBranch* b)    {
	...
	if ( nbyte > 0 )    {
	  ...
	  for(size_t i=0; i<min(hits->size(),10UL); ++i)    {
	    DualCrystalHcalHit* hit = (DualCrystalHcalHit*)hits->at(i);
  ...
  int Dump::DualCrystalHcaldumpData(int num_evts, const char* file_name)    {
    if ( !file_name )    {
    ...
    t->Print();
    DualCrystalHcaldump_branch(num_evts,t->GetBranch("CrystalEcalBarrelHits"));
